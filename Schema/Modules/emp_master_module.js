...
const GetEngList = (args) => {
		let sql = `SELECT a.*, b.user_type FROM md_employee a JOIN md_users b ON a.emp_code=b.code_no WHERE user_type IN('A','M','T','E', 'W') AND user_status='A' ORDER BY a.emp_name`;
	return new Promise((resolve, reject) => {
		db.query(sql, (err, result) => {
			if (err) {
                console.log({ msg: err });
