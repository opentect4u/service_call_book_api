...
const ClientTktUpdate = async (args) => {
    ...
    return new Promise(async (resolve, reject) => {
        datetime = dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss");
        var sql = `UPDATE td_support_log SET tkt_module = "${tkt_module}", phone_no = "${phone_no}",  email_id = "${email}", user_name = "${name}", prob_reported = '${prob_reported ? prob_reported.split("'").join("\\'") : ''} : ''}', modified_by = "${user_id}", modified_dt = "${datetime}" WHERE id = "${id}"`;
        // console.log(sql);
        db.query(sql, (err, lastId) => {
            if (err) {
                data = { success: 0, message: JSON.stringify(err) };
...
const ClientCloseTktUpdate = async (args) => {
    const { client_id, client_close_flag, client_closing_remarks, client_closed_rating, user_id, id } = args;
    return new Promise(async (resolve, reject) => {
        datetime = dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss");
        var sql = `UPDATE td_support_log SET closed_by_client = "${client_close_flag}", closed_by_client_datetime = "${datetime}", client_closing_remarks = '${client_closing_remarks ? client_closing_remarks.split("'").join("\\'") : ''}', client_closed_rating = "${client_closed_rating}", modified_by = "${user_id}", modified_dt = "${datetime}" WHERE id = "${id}"`;
        // console.log(sql);
        db.query(sql, (err, lastId) => {
            if (err) {
                data = { success: 0, message: JSON.stringify(err) };
...
module.exports = { ClientGetTkt, ClientTktSave, ClientUploadFile, ClientTktUpdate, ClientCloseTktUpdate };